...
def mi_second_forward(mels, lf0, encoder, encoder_emo, encoder_lf0, cpc, encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net,ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, optimizer, scheduler):
    optimizer.zero_grad()
    ...
    optimizer.step()
    return optimizer, recon_loss, vq_loss, cpc_loss, accuracy, perplexity, mi_cs_loss, mi_ps_loss, mi_cp_loss, mi_ec_loss, mi_se_loss, mi_ep_loss


def calculate_eval_loss(mels, lf0, \
                        encoder, encoder_lf0, cpc, \
                        encoder_spk, encoder_emo, cs_mi_net, ps_mi_net, \
                        cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg):
    with torch.no_grad():
        ...
        return recon_loss, vq_loss, cpc_loss, accuracy, perplexity, mi_cs_loss, lld_cs_loss, mi_ps_loss, lld_ps_loss, \
               mi_cp_loss, lld_cp_loss, mi_ec_loss, lld_ec_loss, mi_se_loss, lld_se_loss, mi_ep_loss, lld_ep_loss
...
def eval_model(epoch, checkpoint_dir, device, valid_dataloader, encoder, encoder_lf0, encoder_emo, cpc, encoder_spk,
               cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg):
    stime = time.time()
    ...
    ctime = time.time()
    print(
        "Eval | epoch:{}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, lld cs loss:{:.3f}, mi cs loss:{:.3E}, lld ps loss:{:.3f}, mi ps loss:{:.3f}, lld cp loss:{:.3f}, mi cp loss:{:.3f}, used time:{:.3f}s"
            .format(epoch, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity,
                    average_lld_cs_loss,
    ...
    results_txt.write(
        "Eval | epoch:{}, recon loss:{:.3f}, cpc loss:{:.3f}, vq loss:{:.3f}, perpexlity:{:.3f}, lld cs loss:{:.3f}, mi cs loss:{:.3E}, lld ps loss:{:.3f}, mi ps loss:{:.3f}, lld cp loss:{:.3f}, mi cp loss:{:.3f}"
        .format(epoch, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity, average_lld_cs_loss,
...
def train_model(cfg):
    ...
    for epoch in range(start_epoch, cfg.training.n_epochs + 1):
        ...
        for i, (mels, lf0, speakers) in enumerate(dataloader, 1):
            ...
            mi_ps_loss, mi_cp_loss, mi_ec_loss, mi_se_loss, mi_ep_loss = mi_second_forward(
                mels, lf0, encoder, encoder_emo, encoder_lf0,cpc, encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net,\
                ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg, optimizer, scheduler)
        ...
        results_txt.write(
            ...
            .format(epoch, global_step, average_recon_loss, average_cpc_loss, average_vq_loss, average_perplexity,
                    average_lld_cs_loss, average_mi_cs_loss, average_lld_ps_loss, average_mi_ps_loss,
                    average_lld_cp_loss, average_mi_cp_loss, average_lld_ec_loss, average_mi_ec_loss, \
                    average_lld_se_loss, average_mi_se_loss, average_lld_ep_loss, average_mi_ep_loss) + '\n')
        results_txt.write(' '.join([str(cpc_acc) for cpc_acc in average_accuracies]) + '\n')
        ...
        if epoch % cfg.training.log_interval == 0 and epoch != start_epoch:
            eval_model(epoch, checkpoint_dir, device, valid_dataloader, encoder, encoder_lf0, encoder_emo, cpc,
                       encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net, ec_mi_net, se_mi_net, ep_mi_net, decoder, cfg)
