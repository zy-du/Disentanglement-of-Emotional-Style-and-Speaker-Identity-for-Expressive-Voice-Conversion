...
class Decoder(nn.Module):
    """Decoder module:
    """
    def __init__(self, dim_neck=64, dim_lf0=1, dim_emb=256, dim_emoemb = 256,dim_pre=512):
        super(Decoder, self).__init__()
        
        self.lstm1 = nn.LSTM(dim_neck+dim_emb+dim_lf0+dim_emoemb, dim_pre, 1, batch_first=True)
...
class Decoder_ac(nn.Module):
    """Decoder_ac network."""
    def __init__(self, dim_neck=64, dim_lf0=1, dim_emb=256, dim_emoemb =256, dim_pre=512, use_l1_loss=False):
        super(Decoder_ac, self).__init__()
        self.use_l1_loss = use_l1_loss
        # self.encoder = Encoder(dim_neck, dim_emb, freq)
        self.decoder = Decoder(dim_neck, dim_lf0, dim_emb, dim_emoemb,dim_pre)
        self.postnet = Postnet()

    def forward(self, z, lf0_embs, spk_embs, emo_embs,mel_target=None):
